...
<body>
  ...
  <div id="commentsSection">
    ...
    <%-- <c:choose>
	    ...
	    <c:forEach items="${board.comment}" var="comment">
	      <div class="comment" id="${comment.cno}">
	        <strong>${comment.member.id}</strong> <span>(<fmt:formatDate value="${comment.cdate}" parttern="yyyy-MM-dd HH:mm"/>)</span>
	    ...
	    <!-- 더보기 버튼: 서버에서 준 isLast가 false일 때만 노출 -->
		<div id="moreContainer" style="text-align:center; margin-top:20px;">
		    <button type="button" id="moreBtn" style="${commentData.isLast ? 'display:none' : 'width:100%; padding:10px; background:#f8f8f8; border:1px solid #ddd; cursor:pointer;'}">
		      댓글 더보기 <i class="fas fa-chevron-down"></i>
		    </button>
		</div>
	    </c:otherwise>
	</c:choose> --%>
    ...
    <!-- 더보기 버튼 영역 -->
	<div id="moreContainer" style="text-align:center; margin: 20px 0;">
	    <button type="button" id="moreBtn" style="display:none; width:100%; padding:12px; border:1px solid #ddd; background:#f9f9f9; cursor:pointer;">
	        댓글 더보기 <i class="fas fa-chevron-down"></i>
	    </button>
	</div>
  </div>
  ...
  <script>
  function getCommentList(lastCno = 0) {
      const bno = "${board.bno}";
      
      $.ajax({
          url: "/comment/list",
          type: "GET",
          data: { bno: bno, lastCno: lastCno },
          dataType: "json",
          success: function(data) {
              ...
              if (data.list && data.list.length > 0) {
                  ...
                  data.list.forEach(function(c) {
                      let date = new Date(c.cdate).toLocaleString('ko-KR');
                      
                      // 리스트 생성 (기존 JSTL 구조와 동일하게)
                      html += `
                          <div class="comment" id="\${c.cno}" data-cno="\${c.cno}">
                              <strong>\${c.member.id}</strong> <span>(\${date})</span>
                              <p>\${c.ccontent}</p>
                              <div class="comment-buttons">
                                  <button class="edit-btn">수정</button>
                                  <button class="delete-btn">삭제</button>
				  ...
				  // 처음 로딩이면 덮어쓰기, 더보기면 추가하기
                  if(lastCno === 0) $("#commentsList").empty();
                  $("#commentsList").append(html);
                  
                  // 마지막 페이지 여부에 따른 버튼 제어
                  if (data.isLast) {
                      $("#moreBtn").hide();
                  } else {
                      $("#moreBtn").show();
                  ...
                  // 가져온 데이터가 아예 없을 때
                  if(lastCno === 0) {
                      $("#commentsList").html('<p class="no-comment">등록된 댓글이 없습니다.</p>');
                  }
                  $("#moreBtn").hide();
              }
          },
  ...
  $("#moreBtn").on("click", function() {
      // 현재 리스트에 있는 마지막 댓글의 data-cno 값을 가져옴
      const lastCno = $(".comment").last().data("cno");
      
      console.log("마지막 cno:", lastCno); // 확인용
      getCommentList(lastCno);
  });
    ...
    commentsList.addEventListener('click', (e) => {
      if (e.target.classList.contains('delete-btn')) {
        ...
        saveBtn.addEventListener('click', () => {
          ...
          //변수 temp만들어서 다른곳 수정중일때 수정안되게 막을 수 있음
          const cno = commentDiv.id;
          $.ajax({
	    		url: '/comment/update',	//save함수 공통으로 빼서 사용해도 됨
	    		type: 'post',
	    		data: {"bno": ${board.bno}, "cno": cno, "ccontent": newText},
	    		success: function(data){
					alert('댓글 수정완료');
